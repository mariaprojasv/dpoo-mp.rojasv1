...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        for( Vuelo otroVuelo : vuelos )
        {
            if( otroVuelo.getAvion( ) == avion && otroVuelo.getFecha( ).equals( fecha ) )
            {
                Ruta otraRuta = otroVuelo.getRuta( );
                int inicioOtro = Ruta.getHoras( otraRuta.getHoraSalida( ) ) * 60 + Ruta.getMinutos( otraRuta.getHoraSalida( ) );
                int finOtro = inicioOtro + otraRuta.getDuracion( );
                if( inicioNuevo < finOtro && inicioOtro < finNuevo )
                    throw new Exception( "El avión " + nombreAvion + " ya está ocupado el " + fecha + " en la ruta " + otraRuta.getCodigoRuta( ) );
            }
        }
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
    	Cliente cliente = getCliente( identificadorCliente );
        if( cliente == null )
            return "El cliente " + identificadorCliente + " no existe";
